...
- Preserve the original poster concept, people count, relative positions, props, background, and composition. The output is the pictured scene only.
...
- Paint through every text card, caption plate, title banner, logo, watermark, and any other lettering. Do not translate, redraw, or relocate text. After removal, continue the people, props, and background through those areas so no blank patches or leftover marks remain.
...
```text
Reimagine the original poster as if this exact variety-show scene existed inside a Rick-and-Morty-inspired American adult animated cartoon. Keep the same scene concept, same number of people, same relative positions, same props/background, and same overall composition, but redraw the people as native cartoon characters rather than preserving photo-real head shapes or applying a filter. Keep each person's recognizable identity cues: [briefly list visible cues such as hairstyle, glasses, pose, expression, clothing, relative size]. Use thick uneven black outlines, flat saturated colors, simplified shading, large uneven eyes, exaggerated mouths and teeth, rubbery facial geometry, and slightly grotesque comedy caricature. The result is a text-free pictured scene. Paint out every text card, logo, caption, title, watermark, and lettering visible in the source: [list each]. Continue the scene behind them so no blank patches or leftover marks remain. Do not add new characters, objects, logos, food, badges, sci-fi elements, or any lettering. Do not change the scene concept; redraw only the existing pictured scene into this cartoon universe.
```
